...
from app.connectors.errors import ConnectorLoaderError
...
class ConnectorLoader:
    ...
    def discover_from_paths(self, paths: list[str], module_prefix: str = "uv_connector_ext") -> list[str]:
        modules = []
    ...
    def discover_from_entrypoints(self, group: str = "tkk_uv.connectors") -> list[str]:
        modules = []
        try:
    ...
    def _load_module(self, module: ModuleType) -> list[str]:
        ...
        if not callable(register_fn):
            raise ConnectorLoaderError(f"register_plugins must be callable in module {module.__name__}")
        register_fn(self.registry)
